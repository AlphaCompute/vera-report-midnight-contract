...
# Never let old keys satisfy checks for this compiler invocation.
mkdir -p "$(dirname "$output")"
build=$(mktemp -d "${output}.build.XXXXXX")
trap 'rm -rf "$build"' EXIT
"$compiler" "${flags[@]}" contracts/midnight/whistleblower.compact "$build"
...
if [[ "${COMPACT_SKIP_ZK:-0}" == 0 ]]; then
  for circuit in submitEvidenceV2 verifySubmissionV2; do
    for kind in prover verifier; do
      [[ -s "$build/keys/$circuit.$kind" ]] || { echo "Missing proving artifact: $circuit.$kind" >&2; exit 1; }
    done
  done
fi

# Publish only after the complete build passes. A skip build cannot retain old keys.
node --input-type=module - "$build" "$output" <<'NODE'
import { rmSync, renameSync } from 'node:fs';
const [build, output] = process.argv.slice(2);
rmSync(output, { recursive: true, force: true });
renameSync(build, output);
NODE
